...
7c. **`.gitignore` `*.<ext>` rules at the root silently swallow assets inside `_extensions/`.** v1.2.0 shipped without `fmup-fonts.html` because the `*.html` build-artefact glob matched it. Negate explicitly (`!_extensions/fmup/fmup-fonts.html`) for any asset whose extension overlaps a build glob; verify with `git ls-files` before tagging.
